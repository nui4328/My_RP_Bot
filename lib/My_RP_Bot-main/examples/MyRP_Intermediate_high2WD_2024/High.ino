void move_fw(int spl, int spr, int en_pos)
  {
    ...
    while(encoderPos < en_pos)       //------------------------>>  เดินหน้าตามค่า  en_pos
      {        
        ...
        if(mcp_f(1) < md_mcp_f(1) || mcp_f(2) < md_mcp_f(2))  //---------->> ตรวจสอบเส้นข้างหน้า
          {
    ...
    while(encoderPos < 850)   ///---------------------------------->>   เดินเข้าไปเช็คเส้น 
      {
        ...
        if(mcp_f(1) < md_mcp_f(1)+10 || mcp_f(2) < md_mcp_f(2)+10)   ///--------------->>  เมื่อมีเส้น
          {
            while(1)
              {
                if(mcp_f(1) < md_mcp_f(1) && mcp_f(2) > md_mcp_f(2)) 
                ...
                else if(mcp_f(1) < md_mcp_f(1)+10 && mcp_f(2) < md_mcp_f(2)+10)
                  {   
                    Motor(-20, -20);delay(20);
                    Motor(1, 1);delay(300);
                    Motor(0, 0);delay(100);
