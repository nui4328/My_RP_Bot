...
uint16_t sensor_pin_B[] = {1,2,3,4,5,6}; 

uint16_t numSensor_4 = 4; 
uint16_t sensor_pin_4[] = {2,3,4,5}; 
...
float error_CC()
    {
        present_position = Position_CC() / ((numSensor - 1) * 10) ;
        setpoint = 50.0;
        errors = setpoint - present_position;   
...
uint16_t Position4()  
   {        
      uint16_t min_sensor_values_F[] = { min_mcp_f(2),min_mcp_f(3),min_mcp_f(4),min_mcp_f(5) }; //ค่าที่อ่านได้น้อยสุดหรือ สีดำ
      uint16_t max_sensor_values_F[] = { max_mcp_f(2),max_mcp_f(3),max_mcp_f(4),max_mcp_f(5) } ; //ค่าที่อ่านได้มากสุด สีขาว                
      bool onLine = false;
      long avg = 0;
      long sum = 0;
      for (uint8_t i = 0; i < 4 ; i++) 
          {              
              long value = map(mcp_f(sensor_pin_4[i]), min_sensor_values_F[i], max_sensor_values_F[i], 1000, 0);                                                                         // จากนั้นก็เก็บเข้าไปยังตัวแป value
      ...
      if (!onLine)        //เมื่อหุ่นยนต์ไม่อยู่หรือไม่เจอเส้นดำ
         {
            if (_lastPosition < (numSensor_4 - 1) * 1000 / 2)  // ถ้าค่าก่อนหน้าที่จะไม่เจอเส้นดำหรือหลุดจะให้ค่านั้นเป็น 0
               {
                  return 2500;
               }
            else                                          //แต่ถ้ามากกว่าแสดงว่าหลุดออกอีกฝั่ง ค่าก็จะเป็น 1000 คุณด้วยจำนวนเซ็นเซอร์
...
float error_4sensor()
    {
        present_position = Position4() / ((4 - 1) * 10) ;
        setpoint = 50.0;
        errors = setpoint - present_position;   
        return errors;
...
uint16_t PositionB()  
   {        
      uint16_t min_sensor_values_B[] = { min_mcp_b(1),min_mcp_b(2),min_mcp_b(3),min_mcp_b(4),min_mcp_b(5),min_mcp_b(6) }; //ค่าที่อ่านได้น้อยสุดหรือ สีดำ
      uint16_t max_sensor_values_B[] = { max_mcp_b(1),max_mcp_b(2),max_mcp_b(3),max_mcp_b(4),max_mcp_b(5),max_mcp_b(6) } ; //ค่าที่อ่านได้มากสุด สีขาว                
      bool onLine = false;
      long avg = 0;
      long sum = 0;
...
int error_B()
{
    ...
    // ค่า P ใช้ข้อผิดพลาดโดยตรง
    P = errors;
    
    // ค่า I สะสมข้อผิดพลาดในแต่ละรอบ
    I += errors;

    // ค่า D ใช้ความต่างระหว่างข้อผิดพลาดปัจจุบันกับก่อนหน้า
    D = errors - previous_error;
    
    // อัปเดตค่าข้อผิดพลาดและค่า I สำหรับการใช้งานในรอบถัดไป
    previous_error = errors;
